...
class ProxyClient(BaseDao):
    ...
    def enviar_comprobante(self, claveacceso, comprobante, ambiente, info_adicional="", reenvio=0, ruc_empresa=""):
        client = self.get_soap_client()
        res = client.service.validarComprobante(claveacceso, comprobante, ambiente, info_adicional, reenvio,
                                                ruc_empresa)
        return res

    def consulta_autorizacion(self, claveacceso, ambiente, ruc_empresa, info_adicional=""):
        client = self.get_soap_client()
